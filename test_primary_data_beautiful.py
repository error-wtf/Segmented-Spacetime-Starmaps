...
def test_primary_data_access():
    """Test PRIMARY data access with beautiful formatting."""
    ...
    checks = [
        ('M_solar', eso_data['M_solar'].min(), eso_data['M_solar'].max(), 0.1, 1e10),
        ('a_m', eso_data['a_m'].min(), eso_data['a_m'].max(), 1e9, 1e17),
        ('e', eso_data['e'].min(), eso_data['e'].max(), 0.0, 1.0),
        ('r_emit_m', eso_data['r_emit_m'].min(), eso_data['r_emit_m'].max(), 1e3, 1e17),
        ('v_tot_mps', eso_data['v_tot_mps'].min(), eso_data['v_tot_mps'].max(), 1e3, 3e8),
    ]
    ...
    all_ok = True
    for col, min_val, max_val, expected_min, expected_max in checks:
        status = " OK " if (expected_min <= min_val and max_val <= expected_max) else "FAIL"
        if status == "FAIL":
            all_ok = False
        print(f"      | {col:12s} | {min_val:11.2e} | {max_val:11.2e} | {status:^6s} |")
    ...
    if passed_tests == total_tests:
        print("  [OK] STATUS: SUCCESS - PRIMARY data validated!")
    else:
        print(f"  [FAIL] STATUS: PARTIAL - {passed_tests}/{total_tests} tests passed")
    ...
    return passed_tests == total_tests, passed_tests, total_tests, eso_data


def show_data_sample(eso_data):
    """Show sample of PRIMARY data with beautiful formatting."""
...
if __name__ == "__main__":
    # Run test
    success, passed, total, eso_data = test_primary_data_access()
    
    if success:
        # Show sample
        show_data_sample(eso_data)
        
        # Final message
        print()
        print("+" + "="*76 + "+")
        print("|" + " "*76 + "|")
        print("|" + "[OK] PHASE 4 COMPLETE - PRIMARY DATA VALIDATED!".center(76) + "|")
        print("|" + " "*76 + "|")
        print("+" + "="*76 + "+")
        print()
        print("  NEXT STEPS:")
        ...
        print("+" + "="*76 + "+")
        print("|" + " "*76 + "|")
        print("|" + f"[FAIL] TEST FAILED - {passed}/{total} tests passed".center(76) + "|")
        print("|" + " "*76 + "|")
        print("+" + "="*76 + "+")
        print()
